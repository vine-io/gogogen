...
const (
	...
	// only universally-accept path delimiter and the only delimeter not
...
// camelCase names. Seee the NameStrategy struct for an explanation of the
// arguments to this constructor.
...
// directly refer to a type, optionally keeping of the import paths
...
//
// For example, if the types is map[string]int, a raw namer will literally
...
//
// The purpose to this complexity is so that you can assign coherent
// side-by-side systems of names for the types. For example, you might want to
// public interface, a private implementation struct, and also to referenc
// literal the type name.
//
// Note that it is safe to call your own Name() function recursively to find
// the names of keys, elemtns, etc. This is because anonymous types can't have
...
// Public/PrivateNamer variables, and modify the memebers you wish to change.
//
...
type NameStrategy struct {
	...
	// pkg/server/frobbing/"proto, then a value of 1 will given a type name
	// of FrobbingFoo, 2 givens ServerFrobbingFoo, etc.
...
// IC ensure the first character is uppercase.
...
func (ns *NameStrategy) Name(t *types.Type) string {
	...
	case types.Interface:
		...
		for _, m := range t.Methods {
			// TODO: include function sigature
...
// import. You can implement yourself or use one in the generation package.
	...
	if t.Name.Package != "" {
		...
		if r.tracker != nil {
			...
			if t.Name.Package == r.pkg {
				...
				name = filepath.Join(t.Name.Package) + "." + t.Name.Name
			}
		}
		r.Names[t] = name
		return name
