...
// Snippet is an attempt to make the template library unable.
// Methods are chainable, and you don't have check Error() until you're all
// done.
type SnippetWriter struct {
	w       io.Writer
	context *Context
	// Left & right delimiters. text/template default to "{{" and "}}"
...
// short templates, with ordinary go logic between--this may by more
...
// once, at the end of your functions.
//
...
//
// You may or may not find it more readable to use the map which with a descriptive
// key, but if you want to pass more that one arg, the map or a custom struct
...
// WithArgs makes a copy of a and adds the given key, value pair.
func (a Args) WithArgs(key, value interface{}) Args {
	a2 := Args{key: value}
	for k, v := range a {
		a2[k] = v
	}
