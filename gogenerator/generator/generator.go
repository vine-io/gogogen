...
type Package interface {
	...
	// Filter should return true if this package cares about the type.
	// Otherwise, this type will be omitted from the ordering for
	// this package.
	...
	// auto-generation notice! Do not include the "package x" line.
	Header(filename string) []byte

	// Generators returns the list of generators for this package. It is
	// allowed for more that one generator to write to the same file.
...
type Generator interface {
	...
	// Filter should return true if this generator cases about this type.
	...
	// You may return nil if you don't need special names. There names will
	...
	// generated per-write. (It's not intended for generator specific
	// initialization! Do that when your Package constructs the
	// Generators.)
	Init(*Context, io.Writer) error

	// Finialize should write finish up functions, and any other context that's
	// generated per-type
	Finalize(*Context, io.Writer) error

	// Finalize should emit an array of variable lines. They will be
	...
	// Imports should return a list of necessary imports. Thay will be
...
type Context struct {
	...
	// might have public names and several naming systems.
	...
	// the default "golang" filetype will be provide
	FileTypes map[string]FileType

	// If true, Execute* calls will just verify that the existing output is
	// correct. (You may set after calling NewContext.)
	Verify bool
...
// the naming system you wish to construct to canonical ordering from.
